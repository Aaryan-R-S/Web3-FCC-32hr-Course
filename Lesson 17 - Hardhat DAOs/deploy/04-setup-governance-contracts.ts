...
const setupContracts: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const proposerTx = await timeLock.grantRole(proposerRole, governor.address) // only governor (deployer) can propose
  await proposerTx.wait(1)
  const executorTx = await timeLock.grantRole(executorRole, ADDRESS_ZERO) // everyone can execute
  await executorTx.wait(1)
  const revokeTx = await timeLock.revokeRole(adminRole, deployer) // we want timelock to be also under governance
